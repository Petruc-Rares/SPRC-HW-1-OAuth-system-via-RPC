...
request_access_token_response execute_request_client(char *user_id, int auto_refresh, CLIENT* clnt) {
	...
	// first consider the response will be positive
	response.fail = 0;
	...
	// user must approve the authorization token
	...
	// invalid authorization token (most probably due to user not approving it)
	if (response.fail == 1) {
...
int check_refresh_last_operation(token access_token, char *user_id, CLIENT* clnt) {
	...
	if (access_token.no_available_operations == 0) {
		int i;
...
void execute_operation_client(char *host, char *filename_operations) {
	...
	while (fgets(buf, BUFSIZE, fp)) {
		...
		if (strncmp(operation, "REQUEST", strlen(operation)) == 0) {
			...
			// add the authenticated user to database of users
			int user_exists = 0;
			...
			// permissions don't really matter as the pdf mentions
			...
			// initially mark token as invalid
			action_param.access_token.no_available_operations = -1;

			// search for the user in database to find its' associated access token
			int i;
			for (i = 0; i < size_database; i++) {
				if (strncmp(user_database[i].user_id, user_id, SIZE_USER_ID) == 0) {
			...
			// check whether refresh must be done or not
			int refresh_done = check_refresh_last_operation(user_database[i].access_token, user_id, clnt);
			if (refresh_done) {
				action_param.access_token = user_database[i].access_token;
			}
