...
SERVER_PARAMS="tests/test$1/userIDs.db tests/test$1/resources.db tests/test$1/approvals.db ${NO_OPERATIONS_TOKEN[$1-1]}"
...
if [[ $1 =~ $numberPattern ]]; then
    ...
    for i in {1..7}
    do
        SERVER_PARAMS="tests/test$i/userIDs.db tests/test$i/resources.db tests/test$i/approvals.db ${NO_OPERATIONS_TOKEN[$i-1]}"
